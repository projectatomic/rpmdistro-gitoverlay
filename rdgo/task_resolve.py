...
class TaskResolve(Task):
    ...
    def _init_srpm_root(self):
        if self._srpm_mock_initialized:
            return
        # Not necessarily unique but eh
        uniqueext = '{0}-{1}'.format(os.getlogin(), os.getpid())
        self._srpm_mock_argv = ['mock', '-r', self._root_mock, '--uniqueext', uniqueext]
        run_sync(self._srpm_mock_argv + ['--install', 'rpm-build', 'redhat-rpm-config'])
        self._srpm_mock_initialized = True
    ...
    def _generate_srpm(self, component, upstream_tag, upstream_rev, upstream_co,
        ...
        self._init_srpm_root()
            
        try:
            rpmbuild_argv = ['rpmbuild']
            for v in ['_sourcedir', '_specdir', '_builddir',
                      '_srcrpmdir', '_rpmdir']:
                rpmbuild_argv.extend(['--define', '%' + v + ' ' + '/distgit'])
            rpmbuild_argv.extend(['-bs', spec_fn])

            with open(distgit_co + '/tmp-rpmbuild-cmd.sh', 'w') as f:
                f.write("""#!/bin/bash
set -xeuo pipefail
cd /distgit
rpmbuild --define '%_sourcedir /distgit' \
         --define '%_specdir /distgit' \
         --define '%_builddir /distgit' \
         --define '%_srcrpmdir /distgit' \
         --define '%_rpmdir /distgit' \
         -bs /distgit/{0}
# Awful hack because we can't just glob into mock for copyout
for f in *.src.rpm; do
  echo $f >> srpms.txt
  break
done
""".format(spec_fn))
            os.chmod(distgit_co + '/tmp-rpmbuild-cmd.sh', 0755)
            # Also, make the whole dir world-execute because otherwise
            # mock's --copyout won't be able to find anything in it
            os.chmod(distgit_co, 0755)

            rpmbuild_cmd = subprocess.list2cmdline(rpmbuild_argv)
            run_sync(self._srpm_mock_argv + ['--copyin', distgit_co, '/distgit'])
            run_sync(self._srpm_mock_argv + ['--shell', '--', '/distgit/tmp-rpmbuild-cmd.sh'])
            run_sync(self._srpm_mock_argv + ['--copyout', '/distgit/srpms.txt', distgit_co])
            with open(distgit_co + '/srpms.txt') as f:
                for line in f:
                    run_sync(self._srpm_mock_argv + ['--copyout', '/distgit/' + line.strip(), distgit_co])
                    break
        finally:
            if not 'PRESERVE_TEMP' in os.environ:
                run_sync(self._srpm_mock_argv + ['--clean'], stderr=open('/dev/null', 'w'))
